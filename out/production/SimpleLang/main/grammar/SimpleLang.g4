...
expression returns [Expr exprRet]
  ...
  | c = Constant {$exprRet = new Constant($c.text, false); $exprRet.setLine($c.line);}
  | s = StringLiteral+ {$exprRet = new Constant($s.text, true); $exprRet.notFirst();}
  ...
  | {PrefixExpr pe = new PrefixExpr();} (pp = PlusPlus {pe.addOp($pp.text); pe.setLine($pp.line);} | mm = MinusMinus {pe.addOp($mm.text); pe.addOp($pp.text); pe.setLine($mm.line);} | s = Sizeof {pe.addOp($s.text); pe.addOp($pp.text); pe.setLine($s.line);})* (                                          // Prefix operators (zero or more)
...
initDeclarator returns [InitDeclarator initDeclaratorRet]
    : d = declarator {$initDeclaratorRet = new InitDeclarator($d.declaratorRet);}
        (a = Assign i = initializer {$initDeclaratorRet.setLine($a.line); $initDeclaratorRet.setInitializer($i.initializerRet);})? ;
...
jumpStatement returns [JumpStmt jumpStmtRet]
    : {$jumpStmtRet = new JumpStmt();}( Continue | Break |
        Return (e = expression { $jumpStmtRet.setReturnExpr($e.exprRet);})? {$jumpStmtRet.setReturn();}) r = Semi {$jumpStmtRet.setLine($r.line);};
