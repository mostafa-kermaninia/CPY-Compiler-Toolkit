...
public class VulnVisitor extends Visitor<Void>{
    ...
    public Void visit(FunctionDefinition functionDefinition) {
        ...
        func_dec_item.type = functionDefinition.getDecSpecifiers().getDeclarationSpecifiers().get(0).getTypeSpecifier().getType();
        try {
    ...
    public Void visit(InitDeclarator initDeclarator) {
        ...
        if (initDeclarator.getDeclarator().getPointer() != null){
            ...
            try {
                ((VarDecSymbolTableItem) SymbolTable.top.getItem(VarDecSymbolTableItem.START_KEY + identifier)).isFree = true;
                ((VarDecSymbolTableItem) SymbolTable.top.getItem(VarDecSymbolTableItem.START_KEY + identifier)).isPtr = true;
        ...
        if (initDeclarator.getInitializer() != null) {
                    ...
                    ((Identifier)((FuncCall) initDeclarator.getInitializer().getExpr()).getExpr()).getIdentifier().equals("malloc"))
            {
                String identifier = initDeclarator.getDeclarator().getDirectDec().getIdentifier();
                try {
                    ((VarDecSymbolTableItem) SymbolTable.top.getItem(VarDecSymbolTableItem.START_KEY + identifier)).isFree = false;
                    ((VarDecSymbolTableItem) SymbolTable.top.getItem(VarDecSymbolTableItem.START_KEY + identifier)).isPtr = true;
                } catch (ItemNotFoundException e) {
                }
            }
            String identifier = initDeclarator.getDeclarator().getDirectDec().getIdentifier();
            try {
                ((VarDecSymbolTableItem) SymbolTable.top.getItem(VarDecSymbolTableItem.START_KEY + identifier)).isInit = true;
            } catch (ItemNotFoundException e) {
            }
    ...
    private Void freePtr(Expr expr){
        ...
        try {
            ((VarDecSymbolTableItem) (SymbolTable.top.getItem(VarDecSymbolTableItem.START_KEY + identifier.getIdentifier()))).isFree = true;
        }catch (ItemNotFoundException e) {
    ...
    public Void visit(FuncCall funcCall) {
        ...
        if (funcName.equals("scanf")){
            ...
            for (Expr expr : argExpr.getExprs()) {
                if (expr instanceof PrefixExpr){
                    ...
                    try{
                        ((VarDecSymbolTableItem) SymbolTable.top.getItem(VarDecSymbolTableItem.START_KEY + identifier)).isInit = true;
                        ((VarDecSymbolTableItem) SymbolTable.top.getItem(VarDecSymbolTableItem.START_KEY + identifier)).fromUser = true;
                    }catch (ItemNotFoundException e){
                        break;
                    }
                }
        ...
        if (funcName.equals("scanf") || funcName.equals("printf") || funcName.equals("malloc") ||
            ...
            try {
                FuncDecSymbolTableItem funcDec = (FuncDecSymbolTableItem) SymbolTable.top.getItem(FuncDecSymbolTableItem.START_KEY  + funcCall.getNumArgs() + funcName );
        ...
        if (funcCall.getArgExpr() != null) {
            for (int i = 0; i < funcCall.getArgExpr().getExprs().size(); i++) {
                ...
                try {
                    ...
                    if (((VarDecSymbolTableItem) funcDef.getSymbolTable().getItem(VarDecSymbolTableItem.START_KEY + identifier)).isFree)
    ...
    public Void visit(Identifier identifier) {
        if (!identifier.isFunc() && !identifier.getIdentifier().startsWith("\"")){
            try {
                if (!((VarDecSymbolTableItem) SymbolTable.top.getItem(VarDecSymbolTableItem.START_KEY + identifier.getIdentifier())).isInit){
                    System.out.println("Line:" + identifier.getLine() + " -> uninitialized variable used");
                    ((VarDecSymbolTableItem) SymbolTable.top.getItem(VarDecSymbolTableItem.START_KEY + identifier.getIdentifier())).isInit = true;
                }
            } catch (ItemNotFoundException e) {
                System.out.println("Line:" + identifier.getLine() + "-> " + identifier.getIdentifier() + " not declared");
                noError = false;
